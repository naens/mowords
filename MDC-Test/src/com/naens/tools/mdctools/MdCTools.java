...
public class MdCTools {
	...
	private static MdCElement parseMdCString(String element) {
		if (!element.matches(".*[\\*\\:].*")) {
			return new MdCLetter(convertMdCToCodePoint(element));
		}
	...
	private static int convertMdCToCodePoint(String subElement) {
		if (subElement.length() == 0) {
			return -1;
		}
		//TODO: convert mdc Unicode
		return subElement.charAt(0);
	}
