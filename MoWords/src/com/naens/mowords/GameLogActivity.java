	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		while (!cursor.isAfterLast()) {
			...
			if (side == 2) { // is second side
				delImage.setTag(R.tag.game_log_id2, id);
				delImage.setTag(R.tag.game_log_row1, row);
				...
				delImage.setLayoutParams(params);
				delImage.setTag(R.tag.game_log_row2, row);
				delImage.setImageResource(android.R.drawable.ic_delete);
				delImage.setTag(R.tag.game_log_id1, id);
				Log.i("TAG", "delImage tag = " + id);
					...
					public boolean onLongClick(View v) {
						...
						alertbox.setPositiveButton(positiveMessage, new DialogInterface.OnClickListener() {
							public void onClick(DialogInterface arg0, int arg1) {
								...
								db.close();
								dbHelper.close();
								TableRow row1 = (TableRow) delImage.getTag(R.tag.game_log_row1);
								table.removeView(row1);
								TableRow row2 = (TableRow) delImage.getTag(R.tag.game_log_row2);
								if (row2 != null) {
